...
The inherited presentation, partner artwork and event layout are preserved. The agenda, menu, date, venue and credit offering still need confirmation before the Paris event launches. Database settings `event_name` and `description` override the Paris defaults if present.
